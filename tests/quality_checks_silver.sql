...
-- Checking for customer IDs that are not present in 'silver.crm_cust_info'
-- Expectation: No Results
...
CASE
	...
	ELSE cid
END as cid
FROM bronze.erp_cust_az12 WHERE CASE
	WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid))
	ELSE cid
...
-- Expectation: Birthdates less than Today
...
-- Data Standardization & Consistency
-- Expectation: Countries should not contain null values, unwanted spaces or duplicates (i.e. 'USA' vs 'US' vs 'United States')
